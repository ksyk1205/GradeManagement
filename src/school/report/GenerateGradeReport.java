...
public class GenerateGradeReport {
	...
	public static final String HEADER = "  이름  |  학번  |  종점과목 |  점수 \n";
	...
	public void makeBody(Subject subject) {
		ArrayList<Student> studentList = subject.getStudentList(); //각 과목의 학생
		...
		for(int i=0; i<scoreList.size(); i++) { //학생이 가진 점수들
			...
			if(score.getSubject().getSubjectId() == subject.getSubjectId()) { //현재 학점을 산출할 과목
				...
				if(score.getSubject().getGradeType() == Define.PF_TYPE) {
					grade = gradeEvaluation[Define.PF_TYPE].getGrader(score.getPoint());
				}else {
					if(score.getSubject().getSubjectId() == majorId) { //중점 과목인 경우
						grade = gradeEvaluation[Define.SAB_TYPE].getGrader(score.getPoint());//중점 과목 학점 평가 방법
					}else {
						grade = gradeEvaluation[Define.AB_TYPE].getGrader(score.getPoint());//중점 과목이 아닌 경우
					}
				}
